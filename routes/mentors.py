...
def getAllMentors():
    ...
    try:
        if request.method == 'GET':
            ...
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT `uid` FROM tags WHERE MATCH(tag1,tag2,tag3,tag4,tag5,tag6,tag7,tag8,tag9,tag10,tag11,tag12,tag13,tag14,tag15,tag16,tag17,tag18,tag19,tag20) AGAINST ('{_keywords}' IN NATURAL LANGUAGE MODE);")
